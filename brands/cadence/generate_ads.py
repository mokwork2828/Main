#!/usr/bin/env python3
"""
generate_ads.py — Ad Image Generator (Google AI Studio / Imagen 3)
Reads prompts.json, fires each prompt to Imagen 3 via the Google AI Studio API,
...
NUM_IMAGES   = 4     # Imagen 3 supports up to 4 per request
...
def generate_images(prompt_data: dict) -> list[bytes]:
    """Call Imagen 3 and return a list of raw PNG bytes."""
    raw_ratio    = prompt_data.get("aspect_ratio", "1:1")
    aspect_ratio = ASPECT_RATIO_MAP.get(raw_ratio, "1:1")
    if aspect_ratio != raw_ratio:
        print(f"  Aspect ratio {raw_ratio} → mapped to {aspect_ratio} (nearest Imagen 3 support)")
...
def main():
    parser = argparse.ArgumentParser(description="Generate ads via Google AI Studio / Imagen 3")
