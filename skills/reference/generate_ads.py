#!/usr/bin/env python3
"""
generate_[ads.py](http://ads.py) — Nano Banana 2 Ad Image Generator
...
Usage:
    python generate_[ads.py](http://ads.py)                        # all prompts
    python generate_[ads.py](http://ads.py) --templates 1,7,13     # specific templates only
"""
...
def upload_image(image_path: Path) -> str:
    ...
    print(f"  Uploaded {image_[path.name](http://path.name)} → {url}")
    return url
...
def submit_job(endpoint: str, payload: dict) -> str:
    ...
    resp = [requests.post](http://requests.post)(url, headers=HEADERS, json=payload, timeout=30)
    resp.raise_for_status()
...
def save_prompt_results(prompt_data: dict, images: list[dict], outputs_dir: Path) -> list[Path]:
    ...
    for i, img in enumerate(images, start=1):
        ...
        print(f"  Downloading image {i}/{len(images)} → {[dest.name](http://dest.name)}")
...
def build_gallery(brand: str, generated_at: str, outputs_dir: Path) -> None:
    ...
    for folder in sorted(outputs_dir.iterdir()):
        if not [folder.is](http://folder.is)_dir():
            continue
        ...
        cards = "\n".join(
            f'<div class="card"><img src="{img.relative_to(outputs_dir.parent)}" loading="lazy"><span>{[img.name](http://img.name)}</span></div>'
            for img in imgs
        )
        sections.append(
            f'<div class="template"><h2>{[folder.name](http://folder.name)}</h2>'
